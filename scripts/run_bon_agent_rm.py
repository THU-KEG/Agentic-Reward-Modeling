...
def main():
    ...
    def process_item(step, item, reward_agent, reward_pipeline_kwargs, args):
        ...
        answers = item["responses"][:args.n]
        chosen = reward_agent.judge_multi_avg(instruction, answers, **{"reward_pipeline_kwargs": reward_pipeline_kwargs, "force_constraint_check": args.force_constraint_check, "force_factuality_check": args.force_factuality_check})
        return {
            "prompt": instruction,
            "response": chosen
        }

    # 使用ThreadPoolExecutor进行并行化处理
    import concurrent
    results = []
    with concurrent.futures.ThreadPoolExecutor(max_workers=args.num_threads) as executor:
        futures = []
        for step, item in enumerate(tqdm(dataset, desc="RM batch steps")):
    ...
    with open(os.path.join(output_dir, f"results_best_of_{args.n}.jsonl"), "w") as f:
        for item in results:
